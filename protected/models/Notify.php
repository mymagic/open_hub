...
class Notify extends NotifyBase
{
	...
	protected function beforeSave()
	{
		...
		{
			if($this->hasSms && !$this->sentSms) $this->is_sent = 0;
			if($this->hasEmail && !$this->sentEmail) $this->is_sent = 0;
			if($this->hasPush && !$this->sentPush) $this->is_sent = 0;
	...
	public function hasSentAll()
	{
		...
		if($hasSms && $sentSms) $tmp++;
		if($hasEmail && $sentEmail) $tmp++;
		if($hasPush && $sentPush) $tmp++;
		if($fufill == $tmp) $isSent = 1;
