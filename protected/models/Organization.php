...
class Organization extends OrganizationBase
{
	...
	public function beforeValidate() 
	{
		...
		else
		{
			...
			// UUID
			if(empty($this->code)) $this->code = strtolower(ysUtil::generateUUID());	
		}
	...
	public function canAccessByUserEmail($userEmail)
	{
		foreach($this->organization2Emails as $item)
		{
			if($userEmail==$item->user_email && $item->status=='approve') return true;
		}
		return false;
	}
	...
	{
		if($this->image_logo == $this->getDefaultImageLogo()) return true;
		return false;
	}
	...
	public function addEventOrganization($eventCode, $asRoleCode, $extra='')
	{
		if($this->hasNoEventOrganization($eventCode, $asRoleCode))
		{
			...
			if(!empty($extra) && !empty($extra['eventId'])) $eo->event_id = $extra['eventId'];
			if(!empty($extra) && !empty($extra['eventVendorCode'])) $eo->event_vendor_code = $extra['eventVendorCode'];
			if(!empty($extra) && !empty($extra['registrationCode']))$eo->registration_code = $extra['registrationCode'];
			if(!empty($extra) && !empty($extra['dateAction']))$eo->date_action = $extra['dateAction'];
	...
	public function addIndividualOrganization($individualId, $asRoleCode, $extra='')
	{
		if($this->hasNoIndividualOrganization($individualId, $asRoleCode))
		{
			...
			if(!empty($extra) && !empty($extra['jobPosition'])) $eo->job_position = $extra['jobPosition'];
			if(!empty($extra) && !empty($extra['dateStarted'])) $eo->date_started = $extra['dateStarted'];
			if(!empty($extra) && !empty($extra['dateEnded'])) $eo->date_ended = $extra['dateEnded'];
	...
	public function getForeignReferList($isNullable=false, $is4Filter=false, $htmlOptions='')
	{
		...
		else if(!empty($htmlOptions['params']) && !empty($htmlOptions['params']['mode']) && $htmlOptions['params']['mode']=='ideaEnterpriseCode')
		{
			$sql = sprintf("SELECT o.code as `key`, o.title as `title` FROM `organization` as o 
		...
		else if(!empty($htmlOptions['params']) && !empty($htmlOptions['params']['mode']) && $htmlOptions['params']['mode']=='code')
		{
			$result = Yii::app()->db->createCommand()->select("code as key, title as title")->from(self::tableName())->order(array('title ASC'))->queryAll();
		}
		else if(!empty($htmlOptions['params']) && !empty($htmlOptions['params']['mode']) && $htmlOptions['params']['mode']=='isActiveId')
		{
	...
	public function calcProfileCompletenessScore()
	{
		...
		if(!empty($this->text_short_description)) $score += 15;
		else
		{
		...
		// industries
		if(!empty($this->inputIndustries)) $score += 10;
		else
		{
	...
	public function getPublicDisplayStatus($outputFormat='text')
	{
		if($this->is_active)
		{
			...
			{
				if($organizationStatus->status == 'inactive') return Yii::t('app', 'Inactive');
			}
