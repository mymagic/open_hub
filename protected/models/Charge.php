...
class Charge extends ChargeBase
{
	...
	{
		if($this->status == 'pending' || $this->status == 'new') return true;
		return false;
	}
	...
	{
		// expired
		if(HUB::now() > $this->date_expired) return false;
		// havent started
		if(HUB::now() < $this->date_started) return false;
	...
	{
		if($this->status == 'paid') return 'primary';
		if($this->status == 'cancel') return 'danger';
	...
	{
		if($this->charge_to == 'email') return sprintf('%s', $this->charge_to_code);
