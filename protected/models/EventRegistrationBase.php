 ...
 */
 class EventRegistrationBase extends ActiveRecordBase
 {
 	...
 	public $sdate_registered;

 	public $edate_registered;
 	public $sdate_payment;
 	public $edate_payment;
 	public $sdate_added;
 	public $edate_added;
 	...
 	protected function beforeSave()
 	{
 		if (parent::beforeSave()) {
 			...
 			if (empty($this->full_name) && $this->full_name != 0) {
 				$this->full_name = null;
 			}
 			if (empty($this->first_name) && $this->first_name != 0) {
 				$this->first_name = null;
 			}
 			if (empty($this->last_name) && $this->last_name != 0) {
 				$this->last_name = null;
 			}
 			if (empty($this->email) && $this->email != 0) {
 				$this->email = null;
 			}
 			if (empty($this->phone) && $this->phone != 0) {
 				$this->phone = null;
 			}
 			if (empty($this->organization) && $this->organization != 0) {
 				$this->organization = null;
 			}
 			if (empty($this->age_group) && $this->age_group != 0) {
 				$this->age_group = null;
 			}
 			if (empty($this->where_found) && $this->where_found != 0) {
 				$this->where_found = null;
 			}
 			if (empty($this->persona) && $this->persona != 0) {
 				$this->persona = null;
 			}
 			if (empty($this->paid_fee) && $this->paid_fee != 0) {
 				$this->paid_fee = null;
 			}
 			...
 			if (empty($this->json_original) && $this->json_original != 0) {
 				$this->json_original = null;
 			}
