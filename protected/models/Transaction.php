...
class Transaction extends TransactionBase
{
	...
	{
		if ($this->status == 'Approved' && $this->vendor == 'securePay') {
			return true;
		}
	...
	public function getOneLinerDetail()
	{
		if ($this->vendor == 'securePay') {
			if ($this->txntype == 'Receipt') {
				return sprintf('%s %s', $this->jsonArray_payload->cardtype, $this->jsonArray_payload->pan);
			}
			if ($this->txntype == 'Refund') {
				return sprintf('%s %s (%s)', $this->jsonArray_payload->CreditCardInfo->cardDescription, $this->jsonArray_payload->CreditCardInfo->pan, Yii::t('app', 'Refund'));
			}
		}
		// todo: paypal
	}
	...
	{
		if ($this->vendor == 'securePay' && $this->txntype == 'Receipt') {
			return $this->status;
		}
	...
	public function isFail()
	{
		if ($this->vendor == 'securePay') {
			...
			if ($this->txntype == 'Receipt') {
				if (in_array($this->jsonArray_payload->rescode, $arraySuccess)) {
					return false;
				}
			}
			if ($this->txntype == 'Refund') {
				if (in_array($this->jsonArray_payload->responseCode, $arraySuccess)) {
					return false;
				}
			}
		} elseif ($this->vendor == 'paypal') {
