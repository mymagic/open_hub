...
// save as null if empty
					if(empty($this->orid)) $this->orid = null;
						...
						if(empty($this->typefor)) $this->typefor = null;
						if(empty($this->owner)) $this->owner = null;
	...
	{
		// boolean
		if($this->is_featured != '' || $this->is_featured != null) $this->is_featured = intval($this->is_featured);
		if($this->is_active != '' || $this->is_active != null) $this->is_active = intval($this->is_active);
		if($this->is_blocked != '' || $this->is_blocked != null) $this->is_blocked = intval($this->is_blocked);

		$this->jsonArray_extra = json_decode($this->json_extra);

		$this->tag_backend = $this->backend->toString();

		foreach($this->organizations as $organization) $this->inputOrganizations[] = $organization->id;
		...
		foreach($this->resourceCategories as $resource_category) $this->inputResourceCategories[] = $resource_category->id;
		foreach($this->resourceGeofocuses as $resource_geofocus) $this->inputResourceGeofocuses[] = $resource_geofocus->id;
	...
	public function getEnumTypefor($isNullable=false, $is4Filter=false)
	{
		...
		if($isNullable) $result[] = array('code'=>'', 'title'=>$this->formatEnumTypefor(''));
