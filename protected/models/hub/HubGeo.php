...
class HubGeo
{
	public static function geocoder2AddressParts($geocoder)
	{
		if(!empty($geocoder) && !empty($geocoder->first()))
		{
			...
			$buffer = '';
			if(!empty($result['line1'])) $buffer = $result['line1'] . ', ';
			if(!empty($result['line2'])) $buffer .= $result['line2'] . ', ';
			if(!empty($result['city'])) $buffer .= $result['city'] . ', ';
			if(!empty($result['zipcode'])) $buffer .= $result['zipcode'] . ', ';
			if(!empty($result['state'])) $buffer .= $result['state'] . ', ';
			if(!empty($result['countryName'])) $buffer .= $result['countryName'];
	...
	public static function ip2LatLng($ip, $countryCode='')
	{
		...
		if(!empty($result)) return array('lat'=>$result->getLatitude(), 'lng'=>$result->getLongitude());
		return false;
	}
	...
	public static function loc2Address($loc) 
	{
		...
		try
		{
			...
			if(!empty($result->getStreetNumber())) $tmpResult[] = $result->getStreetNumber();
			if(!empty($result->getStreetName())) $tmpResult[] = $result->getStreetName();
			if(!empty($result->getLocality())) $tmpResult[] = $result->getLocality();
			if(!empty($result->getSubLocality())) $tmpResult[] = $result->getSubLocality();
			if(!empty($result->getPostalCode())) $tmpResult[] = $result->getPostalCode();
			if(!empty($result->getAdminLevels()->first()->getName())) $tmpResult[] = $result->getAdminLevels()->first()->getName();
			if(!empty($result->getCountry()->getName())) $tmpResult[] = $result->getCountry()->getName();
