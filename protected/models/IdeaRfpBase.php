 ...
 */
 class IdeaRfpBase extends ActiveRecordBase
 {
 	...
 	public $sdate_transacted;
 	public $edate_transacted;
 	...
 	protected function beforeSave()
 	{
 		if (parent::beforeSave()) {
 			...
 			if (empty($this->text_background)) {
 				$this->text_background = null;
 			}
 			if (empty($this->text_scope)) {
 				$this->text_scope = null;
 			}
 			if (empty($this->text_schedule)) {
 				$this->text_schedule = null;
 			}
 			if (empty($this->text_staff)) {
 				$this->text_staff = null;
 			}
 			if (empty($this->text_cost)) {
 				$this->text_cost = null;
 			}
 			if (empty($this->text_supporting)) {
 				$this->text_supporting = null;
 			}
 			if (empty($this->date_transacted) && $this->date_transacted != 0) {
 				$this->date_transacted = null;
 			}
 			if (empty($this->amount) && $this->amount != 0) {
 				$this->amount = null;
 			}
 			if (empty($this->amount_local) && $this->amount_local != 0) {
 				$this->amount_local = null;
 			}
 			if (empty($this->currency)) {
 				$this->currency = null;
 			}
 			if (empty($this->amount_convert_rate) && $this->amount_convert_rate != 0) {
 				$this->amount_convert_rate = null;
 			}
