 ...
 class OrganizationFundingBase extends ActiveRecordBase
 {
 	...
 	protected function beforeSave()
 	{
 		if (parent::beforeSave()) {
 			if ($this->round_type_code == '') {
 				$this->round_type_code = null;
 			}
 			if ($this->is_amount_undisclosed == '') {
 				$this->is_amount_undisclosed = null;
 			}
 			if ($this->is_publicized == '') {
 				$this->is_publicized = null;
 			}
 			...
 			if (empty($this->date_raised) && $this->date_raised !== 0) {
 				$this->date_raised = null;
 			}
 			if (empty($this->vc_organization_id) && $this->vc_organization_id !== 0) {
 				$this->vc_organization_id = null;
 			}
 			if (empty($this->vc_name)) {
 				$this->vc_name = null;
 			}
 			if (empty($this->is_amount_undisclosed) && $this->is_amount_undisclosed !== 0) {
 				$this->is_amount_undisclosed = null;
 			}
 			if (empty($this->amount)) {
 				$this->amount = null;
 			}
 			if (empty($this->round_type_code)) {
 				$this->round_type_code = null;
 			}
 			if (empty($this->source)) {
 				$this->source = null;
 			}
 			if (empty($this->is_publicized) && $this->is_publicized !== 0) {
 				$this->is_publicized = null;
 			}
 	...
 	protected function afterFind()
 	{
 		...
 		if ($this->is_amount_undisclosed != '' || $this->is_amount_undisclosed != null) {
 			$this->is_amount_undisclosed = intval($this->is_amount_undisclosed);
 		}
 		if ($this->is_active != '' || $this->is_active != null) {
 			$this->is_active = intval($this->is_active);
 		}
 		if ($this->is_publicized != '' || $this->is_publicized != null) {
 			$this->is_publicized = intval($this->is_publicized);
 		}
 	...
 	public function getEnumRoundTypeCode($isNullable = false, $is4Filter = false)
 	{
 		...
 		if ($isNullable) {
 			$result[] = array('code' => '', 'title' => $this->formatEnumRoundTypeCode(''));
 		}
