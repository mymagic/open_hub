...
/**
 * This is the model class for table "startup_stage".
 *
 * The followings are the available columns in table 'startup_stage':
			 * @property integer $id
 ...
 */
 class StartupStageBase extends ActiveRecordBase
 {
 	public $uploadPath;
 	...
 	public function rules()
 	{
 		...
 		return array(
			array('code, slug, title, text_short_description, is_active, title_en, title_ms, text_short_description_en, text_short_description_ms', 'required'),
			...
			array('id, code, slug, title, text_short_description, ordering, is_active, date_added, date_modified, title_en, title_ms, text_short_description_en, text_short_description_ms, sdate_added, edate_added, sdate_modified, edate_modified', 'safe', 'on' => 'search'),
		);
 	}
 	...
 	public function relations()
 	{
 		...
 		return array(
			'events' => array(self::MANY_MANY, 'Event', 'event2startup_stage(startup_stage_id, event_id)'),
			'resources' => array(self::MANY_MANY, 'Resource', 'resource2startup_stage(startup_stage_id, resource_id)'),
			'intakes' => array(self::MANY_MANY, 'Intake', 'startup_stage2intake(startup_stage_id, intake_id)'),
		);
 	}
 	 ...
 	 * @return StartupStage the static model class
 	 */
 	...
 	{
 		$exists = StartupStage::model()->find('code=:code', array(':code' => $code));
 		if ($exists === null) {
 			return false;
 		}
