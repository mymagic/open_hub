...
class BackendController extends Controller
{
    ...
    public function actionConnect()
    {
        ...
        if ($total - $countConnected < 1) {
            ...
            Notice::page(
                Yii::t('notice', 'Out of {total} total users, you have total of {connected} users that registered with MaGIC Connect. Do you like to create MaGIC connect account for the other {notConnected} users?',
                    ['{total}' => $total, '{connected}' => $countConnected, '{notConnected}' => $total - $countConnected]
                ),
                Notice_WARNING, array('url' => $this->createUrl('backend/connectConfirmed'), 'cancelUrl' => $this->createUrl('backend/dashboard'))
            );
        }
    }
    ...
    public function actionGetSystemActFeed($dateStart, $dateEnd, $forceRefresh = 0)
    {
        ...
        try {
            $response = $client->post('getSystemActFeed',
            [
                'form_params' => [
    ...
    public function actionGetSystemLog($page = 1)
    {
        ...
        try {
            $response = $client->post('getEsLogs',
            [
                'form_params' => [
                    'page' => $page,
                ],
