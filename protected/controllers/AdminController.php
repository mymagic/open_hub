...
class AdminController extends Controller
{
	...
	public function actionResetPassword($id)
	{
		...
		if($admin->user->hasRole('superAdmin') && !$this->user->hasRole('superAdmin')) {Notice::page(Yii::t('notice', 'You cannot reset password of an admin with super admin role, without having a super admin role by yourself'));}
	
		Notice::page(Yii::t('notice', "Are you sure to reset password for admin user '{username}'?", ['{username}'=>$admin->user->username]), Notice_WARNING, 
	...
	{
		$admin = $this->loadModel($id);
		if($admin->user->hasRole('superAdmin') && !$this->user->hasRole('superAdmin')) {Notice::page(Yii::t('notice', 'You cannot block an admin with super admin role, without having a super admin role by yourself'));}
		
		if($admin->user->is_active == 1)
		{
			Notice::page(Yii::t('notice', "Are you sure to block this admin user '{username}'? Blocked admin user will not beable to login.", ['{username}'=>$admin->user->username]), Notice_WARNING, 
	...
	{
		$admin = $this->loadModel($id);
		if($admin->user->hasRole('superAdmin') && !$this->user->hasRole('superAdmin')) {Notice::page(Yii::t('notice', 'You cannot unblock an admin with super admin role, without having a super admin role by yourself'));}
		
		if($admin->user->is_active == 0)
		{
			Notice::page(Yii::t('notice', "Are you sure to unblock this admin user '{username}'? Unblocked admin user is active and will beable to login again.", ['{username}'=>$admin->user->username]), Notice_WARNING, 
	...
	{
		$admin = $this->loadModel($id);
		if($admin->user->hasRole('superAdmin') && !$this->user->hasRole('superAdmin')) {Notice::page(Yii::t('notice', 'You cannot remove role from an admin with super admin role, without having a super admin role by yourself'));}
		
		if($admin->user->hasRole($roleCode))
		{
			Notice::page(Yii::t('notice', "Are you sure to remove role '{role}' from admin user '{username}'? Admin user without a specific role will not beable to perform certain tasks.", ['{role}'=>$roleCode, '{username}'=>$admin->user->username]), Notice_WARNING, 
	...
	public function actionAddRole($id, $roleCode)
	{
		...
		{
			Notice::page(Yii::t('notice', "Are you sure to add role '{role}' to admin user '{username}'? Admin user with a specific role will beable to perform certain tasks.", ['{role}'=>$roleCode, '{username}'=>$admin->user->username]), Notice_WARNING, 
	...
	public function actionCreateConnect()
	{
		...
		if(isset($_POST['Admin']))
		{
			...
			if($result == true)
			{
				...
				Notice::page
				(
					Yii::t('notice', "Successfully created admin account with username '{username}'. Login password has been sent to the desinated email address '{email}'. Please remember to add role to the admin user.", array('{username}'=>$model->username, '{email}'=>$model->username)), Notice_SUCCESS,
