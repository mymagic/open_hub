...
class FrontendController extends Controller 
{
	...
	{		
		...
		$persona = ''; $stage = ''; $industry = '' ; $location = ''; $cat = '';
			
		if(isset($_GET['keyword'])){$keyword =  Yii::app()->request->getParam('keyword');}
		if(isset($_GET['persona'])){$persona =  implode(",", Yii::app()->request->getParam('persona'));}
		if(isset($_GET['stage'])){$stage =  implode(",", Yii::app()->request->getParam('stage'));}
		if(isset($_GET['industry'])){$industry =  implode(",", Yii::app()->request->getParam('industry'));}         
		if(isset($_GET['cat'])){$cat =  implode(",", Yii::app()->request->getParam('cat'));}
		if(isset($_GET['location'])){$location =  implode(",", Yii::app()->request->getParam('location'));}
		...
		$viewFile = 'index';
		if($this->layoutParams['brand'] == 'psk') $viewFile = 'index-psk';
	...
	public function actionViewBySlug($slug)
	{
		...
		if($resource == null || !$resource->is_active || $resource->is_blocked) Notice::page(Yii::t('notice','Resource not found'));
	...
	public function actionOrganization($id)
	{
		...
		if($organization == null) Notice::page(Yii::t('notice','Organization not found'));
	...
	public function actionAdd($keyword='')
	{
		...
		if(!empty($_GET) )
		{
			...
			if(isset($_GET['persona'])){$persona =  implode(",",$_GET['persona']);}
			if(isset($_GET['stage'])){$stage =  implode(",",$_GET['stage']);}
			if(isset($_GET['industry'])){$industry =  implode(",",$_GET['industry']);}         
			if(isset($_GET['cat'])){$cat =  implode(",",$_GET['cat']);}
			if(isset($_GET['location'])){$location =  implode(",",$_GET['location']);}
	...
	public function actionCreateResource($id)
	{
		...
		if(!empty($id)) $organization = Organization::model()->findByPk($id);
	...
	public function actionGo($id)
	{
		...
		if($goToWebsite->url_website == null) Notice::page('Website not found');
