...
class GeofocusController extends Controller
{
	...
	{
		$this->redirect(array('resourceGeofocus/admin'));
	}
	 ...
	 * @return ResourceGeofocus the loaded model
	 ...
	 * @param ResourceGeofocus $model the model to be validated
	 */
	...
	public function actionDeactivate($id)
	{
		...
		if ($model->is_active == 1) {
			...
			$this->redirect(array('geofocus/view', 'id' => $id));
		}
	}
	...
	public function actionDeactivateConfirmed($id)
	{
		...
		if ($model->save()) {
			Yii::app()->esLog->log(sprintf("deactivated Resource Geofocus '#%s - %s'", $model->id, $model->title), 'geofocus', array('trigger' => 'GeofocusController::actionDeactivateConfirmed', 'model' => 'Geofocus', 'action' => 'deactivateConfirmed', 'id' => $model->id));

			Notice::flash(Yii::t('notice', "Resource Geofocus '{title}' is successfully deactivated.", ['{title}' => $model->title]), Notice_SUCCESS);
		} else {
			Notice::flash(Yii::t('notice', "Failed to deactivate Resource Geofocus '{title}' due to unknown reason.", ['{title}' => $model->title]), Notice_ERROR);
		}
	...
	public function actionActivateConfirmed($id)
	{
		...
		if ($model->save()) {
			Yii::app()->esLog->log(sprintf("activated Resouce Geofocus '#%s - %s'", $model->id, $model->title), 'geofocus', array('trigger' => 'GeofocusController::actionActivateConfirmed', 'model' => 'Geofocus', 'action' => 'activateConfirmed', 'id' => $model->id));

			Notice::flash(Yii::t('notice', "Resource Geofocus '{title}' is successfully activated.", ['{title}' => $model->title]), Notice_SUCCESS);
		} else {
			Notice::flash(Yii::t('notice', "Failed to activate Resource Geofocus '{title}' due to unknown reason.", ['{title}' => $model->title]), Notice_ERROR);
		}
