...
return parent::afterSave();
		...
		if(parent::beforeSave()) {
			...
			// save as null if empty
					if(empty($this->organization_name)) $this->organization_name = null;
						...
						if(empty($this->month_start)) $this->month_start = null;
						if(empty($this->year_end) && $this->year_end !==0) $this->year_end = null;
						if(empty($this->month_end)) $this->month_end = null;
	...
	public function getEnumGenre($isNullable=false, $is4Filter=false)
	{
		...
		if($isNullable) $result[] = array('code' => '', 'title' => $this->formatEnumGenre(''));
	...
	{
		if($is4Filter) $isNullable = false;
		if($isNullable) $result[] = array('code' => '', 'title' => $this->formatEnumMonthStart(''));
	...
	{
		if($is4Filter) $isNullable = false;
		if($isNullable) $result[] = array('code' => '', 'title' => $this->formatEnumMonthEnd(''));
