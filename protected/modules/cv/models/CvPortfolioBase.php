 ...
 class CvPortfolioBase extends ActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			...
			'jobpos' => array(self::BELONGS_TO, 'CvJobposGroup', 'jobpos_id'),
	...
	public function search()
	{
		...
		$criteria->compare('jobpos_id',$this->jobpos_id);
...
return parent::afterSave();
	...
	protected function beforeSave()
	{
		if(parent::beforeSave()) {
			if($this->state_code == '') $this->state_code = NULL;
			...
			if($this->json_social == 'null') $this->json_social = null;

			// save as null if empty
					if(empty($this->slug)) $this->slug = null;
						if(empty($this->jobpos_id) && $this->jobpos_id !==0) $this->jobpos_id = null;
						if(empty($this->organization_name)) $this->organization_name = null;
						if(empty($this->location)) $this->location = null;
						if(empty($this->text_address_residential)) $this->text_address_residential = null;
						if(empty($this->latlong_address_residential) && $this->latlong_address_residential !==0) $this->latlong_address_residential = null;
						if(empty($this->state_code)) $this->state_code = null;
						if(empty($this->country_code)) $this->country_code = null;
						if(empty($this->image_avatar)) $this->image_avatar = null;
						if(empty($this->high_academy_experience_id) && $this->high_academy_experience_id !==0) $this->high_academy_experience_id = null;
						if(empty($this->text_oneliner)) $this->text_oneliner = null;
						if(empty($this->text_short_description)) $this->text_short_description = null;
						if(empty($this->visibility)) $this->visibility = null;
						if(empty($this->json_social)) $this->json_social = null;
	...
	{
		// boolean
		if($this->is_looking_fulltime != '' || $this->is_looking_fulltime != null) $this->is_looking_fulltime = intval($this->is_looking_fulltime);
		if($this->is_looking_contract != '' || $this->is_looking_contract != null) $this->is_looking_contract = intval($this->is_looking_contract);
		if($this->is_looking_freelance != '' || $this->is_looking_freelance != null) $this->is_looking_freelance = intval($this->is_looking_freelance);
		if($this->is_looking_cofounder != '' || $this->is_looking_cofounder != null) $this->is_looking_cofounder = intval($this->is_looking_cofounder);
		if($this->is_looking_internship != '' || $this->is_looking_internship != null) $this->is_looking_internship = intval($this->is_looking_internship);
		if($this->is_looking_apprenticeship != '' || $this->is_looking_apprenticeship != null) $this->is_looking_apprenticeship = intval($this->is_looking_apprenticeship);
	...
	public function getEnumVisibility($isNullable=false, $is4Filter=false)
	{
		...
		if($isNullable) $result[] = array('code' => '', 'title' => $this->formatEnumVisibility(''));
