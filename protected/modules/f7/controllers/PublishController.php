...
class PublishController extends Controller
{
	...
	{
        if(empty($slug)) throw new Exception('Request is not correct');

		$model = Form::slug2obj($slug);
		
		$this->pageTitle = Yii::t('app', $model->title);

		if(is_null($model)) throw new Exception('Request is not correct');
    ...
    public function actionSave($slug, $sid='',$eid='')
    {
		...
		if (is_null($model)) throw new Exception(Yii::t('f7', 'The request is not in proper format.'));
		...
		{
			if( $key==='YII_CSRF_TOKEN' || $key==='form_code' || $key==='sid' ) continue;
			...
			if(strtolower($key) === 'startup') $orgTitleSubmittedByUser = $value;
		}
		...
		if(is_null($formData)) $formData = new FormSubmission();
 		$sendEmail = 0;
		if ($formData->status === 'submit' && $status == 'draft')
			$sendEmail = 1; // send draft email (warning email)
		elseif($status === 'submit')
			$sendEmail = 2; // send submit email
	...
	public function actionDownload($downloadFile)
	{
		...
		if(self::isUrlExists($downloadLink) === false) throw new Exception('The requested file does not exist.');
