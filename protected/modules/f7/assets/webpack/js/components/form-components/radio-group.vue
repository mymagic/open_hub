<template>
  <div>
    <div class="form-group" v-if="element">
      ...
      <div class="col-sm-7">
        <div v-for="(option, key) in element.members[1].prop.items" :key="key">
          <label>
            <input type="radio" name="optradio" :checked="option.checked" />
            {{option.text}}
          </label>
        </div>
      </div>
              ...
              <div class="col-sm-10">
                <div v-for="(item, key) in element.members[1].prop.items" :key="key">
                  ...
                  <div
                    ...
                    @click="removeSelect(key)"
                    v-if="element.members[1].prop.items.length>1"
                  >×</div>
                </div>
                <br />
...
export default {
  ...
  methods: {
    ...
    addSelect: function() {
      let number = this.element.members[1].prop.items.length + 1;
      this.element.members[1].prop.items.push({
        text: "Option " + number,
        checked: 1
      });
    }
