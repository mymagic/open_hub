...
class HubEventbrite
{
    ...
    protected static function buildEventInsertUpdateSql($event)
    {
        ...
        if (!empty($event['venue']['name'])) {
            $sql = sprintf(
                ...
                $event['id'], Yii::app()->db->quoteValue($event['name']['text']), Yii::app()->db->quoteValue($event['description']['text']), strtotime($event['start']['utc']), strtotime($event['end']['utc']), Yii::app()->db->quoteValue($event['venue']['name']), Yii::app()->db->quoteValue(''), Yii::app()->db->quoteValue($event['venue']['address']['localized_address_display']), $addressCountryCode, ($latlong), Yii::app()->db->quoteValue('eventbrite'), $event['id'], Yii::app()->db->quoteValue(json_encode($event)), $isActive, time(), time()
            );
        } else {
            $sql = sprintf(
                ...
                ON DUPLICATE KEY UPDATE code=VALUES(code), title=VALUES(title), text_short_desc=VALUES(text_short_desc), date_started=VALUES(date_started), date_ended=VALUES(date_ended), at=VALUES(at), email_contact=VALUES(email_contact), full_address=VALUES(full_address), address_country_code=VALUES(address_country_code), latlong_address=VALUES(latlong_address), vendor=VALUES(vendor), vendor_code=VALUES(vendor_code), json_original=VALUES(json_original), is_active=VALUES(is_active), date_modified=VALUES(date_modified); ',
                $event['id'], Yii::app()->db->quoteValue($event['name']['text']), Yii::app()->db->quoteValue($event['description']['text']), strtotime($event['start']['utc']), strtotime($event['end']['utc']), Yii::app()->db->quoteValue(''), Yii::app()->db->quoteValue($event['venue']['address']['localized_address_display']), $addressCountryCode, ($latlong), Yii::app()->db->quoteValue('eventbrite'), $event['id'], Yii::app()->db->quoteValue(json_encode($event)), $isActive, time(), time()
            );
        }
    ...
    public static function syncEventRegistrationFromEventbrite($attendees)
    {
        ...
        foreach ($attendees as $attendee) {
            ...
            /* use for update bumi/indian status when this function is being called */
            if(isset($attendee['id'])) $registration_code[] = $attendee['id'];
        }
    ...
    */
    protected static function buildEventRegistrationInsertUpdateSql($attendee)
    {
        ...
        $sql = sprintf(
            ...
            Yii::app()->db->quoteValue($attendee['event_id']), Yii::app()->db->quoteValue($attendee['event_id']), Yii::app()->db->quoteValue('eventbrite'), Yii::app()->db->quoteValue($ticketId), Yii::app()->db->quoteValue($fullName), Yii::app()->db->quoteValue($attendee['profile']['first_name']), Yii::app()->db->quoteValue($attendee['profile']['last_name']), Yii::app()->db->quoteValue($email), Yii::app()->db->quoteValue($mobileNumber), $attendee['checked_in'] ? '1' : '0', Yii::app()->db->quoteValue($organizationName), Yii::app()->db->quoteValue($gender), Yii::app()->db->quoteValue($ageGroup), Yii::app()->db->quoteValue($whereFound), Yii::app()->db->quoteValue($persona), Yii::app()->db->quoteValue($paidFee), Yii::app()->db->quoteValue($jsonOriginal), strtotime($attendee['created']), ($datePayment), time(), time()
        );
