 ...
 class CommentBase extends ActiveRecordBase
 {
 	...
 	protected function beforeSave()
 	{
 		if (parent::beforeSave()) {
 			...
 			if (empty($this->file_main)) {
 				$this->file_main = null;
 			}
 			if (empty($this->file_main_mimetype)) {
 				$this->file_main_mimetype = null;
 			}
 			if (empty($this->creator_user_id) && $this->creator_user_id !== 0) {
 				$this->creator_user_id = null;
 			}
 			if (empty($this->creator_fullname)) {
 				$this->creator_fullname = null;
 			}
 			if (empty($this->url_creator_profile)) {
 				$this->url_creator_profile = null;
 			}
 			if (empty($this->csv_ping)) {
 				$this->csv_ping = null;
 			}
