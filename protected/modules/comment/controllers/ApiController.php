...
class ApiController extends Controller
{
	...
	public function actionUpvoteComment()
	{
		...
		if (isset($commentId) && !empty($commentId) && !empty($userId)) {
			if (!CommentUpvote::isExists($commentId, $userId)) {
				...
				$upvote = CommentUpvote::model()->findByAttributes(array(), array(
	...
	{
		if ($comment->creator_user_id == Yii::app()->user->id && !empty(Yii::app()->user->id)) {
			return true;
		}
	...
	{
		if (HUB::now() - $comment->date_added < 60 * 60 * 24) {
			return true;
		}
