...
class DefaultController extends CController
{
  ...
  public function actionIndex()
  {
    ...
    if(isset($_POST['SetupForm']))
    {
      ...
      // environment
      // DEV, ENVIRONMENT
      if($model->environment == 'production') $model->envs['DEV'] = var_export(false, true);
      else if($model->environment == 'staging') $model->envs['DEV'] = var_export(true, true);
      else if($model->environment == 'testing') $model->envs['DEV'] = var_export(true, true);
      else if($model->environment == 'development') $model->envs['DEV'] = var_export(true, true);
          ...
          foreach($configGroup['items'] as $configItem)
          {
            if(isset($configItem['env']))
            {
              if(isset($model->envs[$configItem['env']]))
              {
                ...
                else if($configItem['datatype'] == 'string' || $configItem['datatype'] == 'password' || $configItem['datatype'] == 'email' || $configItem['datatype'] == 'url'|| $configItem['datatype'] == 'enum')
                {
  ...
  function sqlImport($file, $mysqli)
  {
